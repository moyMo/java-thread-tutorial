...
public class ThreadLifecycle {
    ...
    public static void main(String[] args) throws InterruptedException {
        log.info("=== Java线程生命周期示例 ===");
        
        // 创建线程但未启动 - NEW状态
        Thread thread = new Thread(new Worker(), "Worker-Thread");
        log.info("线程创建后状态: {}", thread.getState()); // NEW
        
        // 启动线程 - 进入RUNNABLE状态
        thread.start();
        log.info("线程启动后状态: {}", thread.getState()); // RUNNABLE
        
        // 主线程休眠一小会儿，让工作线程运行
        Thread.sleep(50);
        log.info("线程运行中状态: {}", thread.getState()); // TIMED_WAITING (因为Worker中有sleep)
        
        // 等待线程完成 - TERMINATED状态
        thread.join();
        log.info("线程完成后状态: {}", thread.getState()); // TERMINATED
    ...
    static class Worker implements Runnable {
        @Override
        public void run() {
            log.info("工作线程开始执行: {}", Thread.currentThread().getName());
            
            try {
                // 进入TIMED_WAITING状态
                log.info("工作线程进入休眠 (TIMED_WAITING)");
                Thread.sleep(1000);
            ...
            log.info("工作线程执行完成: {}", Thread.currentThread().getName());
        }
    }
    ...
    private static void demonstrateBlockedState() throws InterruptedException {
        ...
        log.info("线程2状态 (等待锁时): {}", thread2.getState()); // BLOCKED
    ...
    private static void demonstrateWaitingState() throws InterruptedException {
        ...
        log.info("等待线程状态 (调用wait()后): {}", waitingThread.getState()); // WAITING
